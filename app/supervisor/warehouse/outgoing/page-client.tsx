...
export function WarehouseOutgoingPageClient({
  ...
  // Live Search State Input Manual
  ...
  useEffect(() => {
    try {
      ...
      if (savedDraft) {
        ...
        if (parsed.scannedItemsMap) setScannedItemsMap(parsed.scannedItemsMap);
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    // Paksa nyalakan kamera belakang langsung tanpa UI permission prompt
    html5Qrcode
      .start(
        { facingMode: "environment" }, // Paksa Kamera Belakang Utama
        {
          fps: 15,
          qrbox: { width: 260, height: 130 }, // Rectangle Bounding Box
        },
        (decodedText) => {
      ...
      .catch((err) => {
        ...
        // Fallback jika HP memblokir { facingMode: "environment" }
        html5Qrcode
          .start(
            { facingMode: "user" },
    ...
    <div className="space-y-4 text-xs font-sans max-w-md mx-auto">
      ...
      <div className="p-3.5 bg-neutral-900 border border-neutral-800 rounded-xl space-y-3">
        ...
        <div className="space-y-2 bg-neutral-950 p-3 rounded-lg border border-neutral-800 font-mono">
          ...
          <div>
            <label className="text-[10px] text-neutral-400 block mb-1">
              CATATAN / NOMOR RESI (OPSIONAL):
            </label>
            <input
              type="text"
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              placeholder="Nomor Resi / Keterangan..."
              className="w-full p-2 bg-neutral-900 border border-neutral-800 text-neutral-100 rounded focus:outline-none"
            />
          </div>
        </div>
      ...
      {isScannerOpen && (
        <div className="relative bg-neutral-950 border border-neutral-800 rounded-xl overflow-hidden p-2 space-y-2">
          ...
          <p className="text-[9.5px] font-mono text-neutral-400 text-center">
            Arahkan Barcode SKU ke area scanner di atas.
          </p>
        </div>
      )}
      ...
      <div className="space-y-2">
          ...
          scannedListUI.map((item: any) => (
            ...
            >
              ...
              <div className="grid grid-cols-3 gap-1 bg-neutral-950 p-2 rounded border border-neutral-800/80 text-[10px] text-center">
                ...
                <div>
                  <span className="text-[8.5px] text-blue-400 block font-bold">
                    QTY KELUAR
                  </span>
                  <div className="flex items-center justify-center gap-1 mt-0.5">
                    ...
                    <span className="font-extrabold text-blue-400 px-1">
                      {item.quantity}
                    </span>
                    <button
        ...
        {scannedListUI.length > 0 && (
          <button
            ...
            className="w-full py-3 bg-blue-600 hover:bg-blue-500 text-neutral-100 font-bold rounded-xl flex items-center justify-center gap-2 uppercase tracking-wider text-xs font-mono disabled:opacity-50 transition-colors shadow-lg mt-2"
          >
            {isSubmitting ? (
            ...
            <span>SIMPAN & POTONG STOK GUDANG</span>
          </button>
        )}
      </div>

      {/* SECTION RIWAYAT BARANG KELUAR */}
      <div className="space-y-2 pt-2 border-t border-neutral-800">
        <div className="flex items-center justify-between px-1 font-mono">
          <span className="text-[10px] font-bold text-neutral-400 uppercase flex items-center gap-1.5">
            ...
            RIWAYAT BARANG KELUAR ({historyList.length})
          </span>
        </div>
